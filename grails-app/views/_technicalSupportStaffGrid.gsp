...
<script type="text/javascript">
    ...
    listTechnicalSupportRoleUrl = "../person/listTechnicalSupport?objectId=${objectId}";

    function getPerson(e) {
        var rowId= e.id.split("_")[0];
        jQuery.ajax({
            async:false,
            url: '../person/getPersonIdFromRoleId',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            data: {supportRoleId: rowId},
    ...
    $(document).ready(function() {
        ...
        $("#btnAddTechnicalSupportStaff").click(function(){
            var myData;
            jQuery.ajax({
                async:false,
            ...
            $("#technicalSupportStaff_list").jqGrid("editGridRow","new",
                    {addCaption:'Create New Technical Support Staff Assignment',
                        width:500,
                        recreateForm:true,
...
//                                closeModal()
        ...
        jQuery("#technicalSupportStaff_list").jqGrid({

            height:'auto',
            width:'1100',
            caption:'Technical Support Staff List',
            showPager:'true',
            url:listTechnicalSupportRoleUrl,
            editurl:"../person/editTechnicalSupport?objectId=${objectId}",
            datatype: "json",
        ...
        colNames:['', 'Role', 'Person', 'E-Mail', 'Phone', 'Notes', 'id'],
            colModel:[
                ...
                {name:'roleName', width:50, editable:true, edittype:'custom', editoptions: {
            ...
            pager: jQuery('#technicalSupportStaff_list_pager'),
            viewrecords: true,
            gridview: true,
            cellEdit:editOption,
            cellsubmit: 'remote',
            afterSaveCell: afterSubmitTechnicalSupportStaffEvent,
            afterSubmit: afterSubmitTechnicalSupportStaffEvent,
            ...
            cellurl:'../person/editTechnicalSupport?objectId=${objectId}'

        });

        /*
        $('#technicalSupportStaff_list').jqGrid('setGridParam', { onSelectRow: function(id){
        ...
        jQuery(window).bind('resize', function() {
            ...
            if (width > 0 &&
                ...
                // Fixes IE issue with in-place resizing when mousing-over frame bars
                    Math.abs(width - jQuery("#technicalSupportStaff_list").width()) > 5)
            {
                jQuery("#technicalSupportStaff_list").setGridWidth(width);
            }
    ...
    /* EVENT FUNCTIONS */
    /*********************************************************/
    function afterSubmitTechnicalSupportStaffEvent(rowId, cellname, value, iRow, iCol) {
        $("#technicalSupportStaff_list").trigger("reloadGrid");
    }


</script>
<table id="technicalSupportStaff_list"></table>

<g:if test="${action=='edit' || action=='create'}">
    <div style="margin-top:5px">
        <input class="ui-corner-all" id="btnAddTechnicalSupportStaff" type="button" value="Add Technical Support Staff Assignment"/>
    </div>
</g:if>
